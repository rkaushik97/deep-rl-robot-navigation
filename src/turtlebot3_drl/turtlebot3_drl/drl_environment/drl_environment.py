...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.debug_pub = (
            self.create_publisher(String, '/drl_debug/step', QoSProfile(depth=10))
            if self.debug_enabled else None
        )
        if self.debug_enabled:
            print("[DEBUG_DRL] env publishing per-step debug JSON on /drl_debug/step")
        # subscribers
    ...
    *******************************************************************************"""
    ...
    def step_comm_callback(self, request, response):
        ...
        if self.debug_pub is not None:
            payload = {
                't_sim': float(self.time_sec),
                'step': int(self.local_step),
                'action_norm': [float(request.action[LINEAR]), float(request.action[ANGULAR])],
                'action_unnorm': [float(action_linear), float(action_angular)],
                'reward': float(response.reward),
                'reward_components': dict(rw.last_components),
                'goal_dist': float(self.goal_distance),
                'goal_angle_deg': float(math.degrees(self.goal_angle)),
                'min_obstacle': float(self.obstacle_distance),
                'succeed': int(self.succeed),
                'robot_xy': [float(self.robot_x), float(self.robot_y)],
                'goal_xy': [float(self.goal_x), float(self.goal_y)],
            }
            msg_str = String()
