...
from ..common.settings import REWARD_FUNCTION, COLLISION_OBSTACLE, COLLISION_WALL, TUMBLE, SUCCESS, TIMEOUT, RESULTS_NUM, SIM_SPEED
...
def get_scan_count():
    # R2: read the lidar sample count from the SDF of the robot we actually spawn,
    # bundled in turtlebot3_drl_gazebo (no turtlebot3_simulations dependency).
    # Handles both <lidar> (gpu_lidar) and legacy <ray> sensors via the //scan path.
    sdf_path = os.path.join(
        get_package_share_directory('turtlebot3_drl_gazebo'),
        'models', 'drl_burger', 'model.sdf')
    root = ET.parse(sdf_path).getroot()
    for sensor in root.iter('sensor'):
        samples = sensor.find('.//scan/horizontal/samples')
        if samples is not None:
            return int(samples.text)
    raise RuntimeError(f"no lidar <samples> found in {sdf_path}")

def get_simulation_speed(stage):
    # R3: the new world is real_time_factor=0 (uncapped). Speed scaling is a
    # settings constant now, not parsed from a vendored per-stage SDF.
    return SIM_SPEED