...
tmux new-session -d -s "$EXP" -n train \
  "bash -c 'source /opt/ros/jazzy/setup.bash; source $BASE/install/setup.bash; \
            export DRLNAV_BASE_PATH=$BASE ROS_DOMAIN_ID=$DOM GZ_PARTITION=$EXP \
                   ...
                   DRL_SIGMA_MAX=${DRL_SIGMA_MAX:-0.1} DRL_SIGMA_MIN=${DRL_SIGMA_MIN:-0.1} DRL_DECAY=${DRL_DECAY:-8000000} \
                   DRL_OBSTACLE_K=${DRL_OBSTACLE_K:-0.5} DRL_OBSTACLE_SAFE=${DRL_OBSTACLE_SAFE:-0.40} DRL_PROGRESS_K=${DRL_PROGRESS_K:-2.0} \
                   DRL_PER=${DRL_PER:-0} DRL_WARMSTART_FILL=${DRL_WARMSTART_FILL:-4000} DRL_NSTEP=${DRL_NSTEP:-1} \
                   DRL_STACKING=${DRL_STACKING:-0} DRL_STACK_DEPTH=${DRL_STACK_DEPTH:-3} DRL_FRAME_SKIP=${DRL_FRAME_SKIP:-4} DRL_LAYERNORM=${DRL_LAYERNORM:-0} \
                   DRL_BATCH_SIZE=${DRL_BATCH_SIZE:-128} DRL_LR=${DRL_LR:-0.003} DRL_TAU=${DRL_TAU:-0.003} DRL_STEP_TIME=${DRL_STEP_TIME:-0.01} DRL_VAL_EPS=${DRL_VAL_EPS:-0} \
                   DRL_BACKWARD=${DRL_BACKWARD:-False} DRL_POLICY_NOISE=${DRL_POLICY_NOISE:-0.2} DRL_POLICY_NOISE_CLIP=${DRL_POLICY_NOISE_CLIP:-0.5} DRL_POLICY_FREQ=${DRL_POLICY_FREQ:-2} \
                   DRL_REWARD_SCALE=${DRL_REWARD_SCALE:-1.0} DRL_SAC_TARGET_ENTROPY_SCALE=${DRL_SAC_TARGET_ENTROPY_SCALE:-0.5} \
